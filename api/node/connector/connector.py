...
def ws_conn():
    if request.sid not in calling_members.values():
        print('Web Service Connect: sid: ' + request.sid + get_display_datetime())
...
def ws_update(data):
    if request.sid not in calling_members.values():
        print('Web Service Update: sid: ' + request.sid + get_display_datetime())
    emit('client_update', data, broadcast=True)

@socketio.on('output_handler_connected')
def ws_output_handler_connected():
    if request.sid not in calling_members.values():
        print('Output Handler Connected: sid: ' + request.sid + get_display_datetime())
...
def ws_updater_connected():
    if request.sid not in calling_members.values():
        print('Updater Connected: sid: ' + request.sid + get_display_datetime())
...
def ws_ready_for_output_handler(data):
    ...
    if calling_members['handler_sid'] == None:
        print('output handler not connected! ' + get_display_datetime())
    ...
    with open(path, "rb") as binary_file:
        ...
        if len(file_chunk) == 0:
            print('End of File ' + get_display_datetime())
            emit('file_saved', {
                'job_name': job_name,
                'file_path': path
...
def output_handler_finished_file_chunk(data):
    ...
    print('Done saving chunk', job_name, file_path, get_display_datetime())
        ...
        if job_type == 'fim_run':
            ...
            if len(validation_errors) == 0:
                for extent in extents:
                    ...
                    print('fim_run job added  ' + get_display_datetime())
            ...
            for hucs, extent in zip(huc_lists, extents):
                ...
                print('release job added print ' + get_display_datetime())
                emit('job_added', 'release')

    except Exception as ex:
        print("An error has occurred while setting up a new job. Exception details: " )
        print(ex)
        print(traceback.format_exc())
        print("Service stopped ")
        emit('service_error')

@socketio.on('updater_service_error')
def updater_error():
    print("The updater service has recorded an error. Please see the updater logs" )
    emit('service_error', broadcast=True)
...
if __name__ == '__main__':
    print('start of main' + get_display_datetime())
